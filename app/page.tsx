...
import { fetchTrades, getMonthWeeks } from "@/lib/notion";
...
export default async function MonthPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Get week ranges for this month
  const weeks = getMonthWeeks(yearNum, monthIndex);
  ...
  for (const t of trades) {
    ...
    for (const w of weeks) {
      if (d >= w.start && d <= w.end) {
        if (!weekMap[w.weekNumber]) weekMap[w.weekNumber] = [];
        weekMap[w.weekNumber].push(t);
        break;
      }
    }
  }
  ...
  // Get monthly note (from first trade's notes, or a dedicated property)
  const monthlyNote = trades[0]?.monthlyNote || "✏️ Tuliskan alur cerita bulan ini di sini.";
        ...
        <div className="wrap">
          ...
          <div className="rail">
            {weeks.map((week) => {
              ...
              return (
                <Link key={week.weekNumber} href={`/month/${slug}/week/${week.weekNumber}`} className="rail-row linked">
                  <div className="rail-day">
                    {monthLabel}<b>{week.start.getDate()}–{week.end.getDate()}</b>
                  </div>
      ...
      <section>
        <div className="wrap">
          ...
          <div style={{ maxWidth: 640, color: "var(--text-muted)", fontSize: 16, lineHeight: 1.85, marginBottom: 60 }}>
            <p>{monthlyNote}</p>
            <p style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--text-faint)", marginTop: 16 }}>
              ✏️ Untuk mengubah catatan ini, tambahkan properti "Monthly Note" di database Notion Anda.
            </p>
          </div>
        </div>
      </section>
